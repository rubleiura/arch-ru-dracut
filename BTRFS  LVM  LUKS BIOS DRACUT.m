...
# ⚠️ Только для макетов с LUKS/LVM!
...
# через dracut для работы с LUKS, LVM, Btrfs и гибернацией.
...
#------------------------------------------------------------------------------
# ℹ️ Создание конфигурации dracut для LUKS + LVM + Btrfs.
# - hostonly="no" гарантирует включение resume и crypt/lvm.
# - Модули crypt, lvm, resume добавлены явно.
#------------------------------------------------------------------------------
# 📋 Создание конфигурационного файла dracut
mkdir -p /etc/dracut.conf.d
cat > /etc/dracut.conf.d/custom.conf << 'EOF'
add_dracutmodules+=" base crypt lvm resume btrfs plymouth "
add_drivers+=" btrfs dm_mod dm_crypt "
hostonly="no"
compress="zstd"
EOF
#------------------------------------------------------------------------------
...
# - Параметры rd.luks, rd.lvm, resume=UUID обязательны для dracut.
#------------------------------------------------------------------------------
...
sed -i "s|^[[:space:]]*#\?GRUB_CMDLINE_LINUX=.*|GRUB_CMDLINE_LINUX=\"rd.luks.uuid=${CRYPT_UUID} rd.lvm.lv=vg_main/lv_root rd.lvm.lv=vg_main/lv_swap root=UUID=${ROOT_UUID} resume=UUID=${SWAP_UUID}\"|" /etc/default/grub
...
echo " "
echo "#####################################################"
echo "## ✅ ЗАГРУЗЧИК И ЯДРО НАСТРОЕНЫ (DRACUT+LUKS+LVM)##"
echo "#####################################################"
echo " "
#################################################################
